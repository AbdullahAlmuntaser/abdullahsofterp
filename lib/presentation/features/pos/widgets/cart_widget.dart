...
<<<<<<< HEAD
                  ...
                  const SizedBox(height: 8),
                  TextFormField(
                    initialValue: item.serialNumbers ?? '',
                    ...
                    onChanged: (v) {
                      context.read<PosBloc>().add(
                            UpdateCartItemSerialNumbers(item.product.id, v),
                          );
                    },
                  ),
=======
      ...
      builder: (ctx) => Column(
        ...
        children: [
          ...
          ListTile(
            ...
            subtitle: const Text('الوحدة الأساسية (المعامل: 1)'),
...
>>>>>>> 2d430f8439a4d864f3ca3b6e9d35a290d925fd86
            ...
            ...filteredUnits.map(
              (u) => ListTile(
                title: Text(u.unitName),
                subtitle: Text('تحتوي على ${u.unitFactor} ${item.product.unit}'),
                trailing: item.unitName == u.unitName
                    ? const Icon(Icons.check, color: Colors.green)
                    : null,
                onTap: () {
...
<<<<<<< HEAD
      ...
      // Validate: cannot create a unit that already exists
      final exists = item.availableUnits.any((u) => u.unitName == unitName);
      if (exists) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      // Validate: factor must be > 1 (otherwise it's equivalent to base unit)
      if (factor <= Decimal.one) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('المعامل يجب أن يكون أكبر من 1 لأنها وحدة تعبئة')),
          );
        }
        return;
      }
