...
class FinancialClosingService {
  ...
  Future<String> _createClosingEntry({
    ...
    final revenueAccounts = allAccounts.where(
      (a) => a.accountType == AccountType.revenue && !a.isHeader,
    );
    final expenseAccounts = allAccounts.where(
      (a) => a.accountType == AccountType.expense && !a.isHeader,
    );
  ...
  Future<void> generateOpeningBalances({
    ...
    if (prevYearPeriod == null) {
      throw NotFoundException(
          message: 'Previous fiscal year $previousYear not found or not closed.');
    }
  ...
  Future<String> createOpeningEntry({
    ...
    final permanentAccounts = allAccounts.where(
      (a) =>
          (a.accountType == AccountType.asset || a.accountType == AccountType.liability || a.accountType == AccountType.equity) &&
          !a.isHeader,
    );
  ...
  Future<ClosingResult> reopenPeriod(
    ...
    if (userId != adminUserId && userId.isNotEmpty) {
      return ClosingResult(
        success: false,
        error: 'فقط مدير النظام يمكنه إعادة فتح الفترات المحاسبية',
        message: '',
        journalEntryId: null,
      );
    }
