...
class BudgetService {
  ...
  /// Validates period format (expected: YYYY-MM)
  void _validatePeriod(String period) {
    final periodRegex = RegExp(r'^\d{4}-\d{2}$');
    if (!periodRegex.hasMatch(period)) {
      throw BusinessException(
        message: 'صيغة الفترة غير صالحة: $period. الصيغة المطلوبة: YYYY-MM',
      );
    }
  }
  ...
  Future<void> validateExpenseAgainstBudget({
    ...
    for (var budget in budgets) {
      ...
      if (expenseAmount > remaining) {
        throw BusinessException(
          message: 'تنبيه: المبلغ المطلوب يتجاوز الميزانية المتبقية لمركز التكلفة ${budget.name}. المتبقي: ${remaining.toStringAsFixed(2)}',
        );
      }
