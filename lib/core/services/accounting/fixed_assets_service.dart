...
class FixedAssetsService {
  ...
  Future<String> _getDepreciationExpenseAccount(int categoryId) async {
    ...
    throw const BusinessException(message: 'لم يتم العثور على حساب مصروف الإهلاك');
  }
  ...
  Future<String> _getAccumulatedDepreciationAccount(int assetId) async {
    ...
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isNotEmpty) return accounts.first.id;
    throw const BusinessException(message: 'لم يتم العثور على حساب مجمع الإهلاك');
  }
  ...
  Future<String> _getCashOrBankAccount() async {
    ...
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب الصندوق');
  ...
  Future<String> _getFixedAssetAccount(int assetId) async {
    ...
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) {
      throw const BusinessException(message: 'لم يتم العثور على حساب الأصول الثابتة');
    }
    return accounts.first.id;
  }
  ...
  Future<String> _getGainOnDisposalAccount() async {
    ...
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب الإيرادات');
    return accounts.first.id;
  }
  ...
  Future<String> _getLossOnDisposalAccount() async {
    ...
    final accounts = await (db.select(db.gLAccounts)
          ..where((t) => t.code.like('600%')))
        .get();
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب المصروفات');
    return accounts.first.id;
  }
