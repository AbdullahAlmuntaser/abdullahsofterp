...
class FinancialReportService {
  ...
  Future<IncomeStatementData> getIncomeStatement({
    ...
    if (startDate != null) {
      ...
      allItems = balances.map((b) {
        if (b.account.accountType == AccountType.revenue) {
    ...
    final revenues =
        allItems.where((i) => i.account.accountType == AccountType.revenue).toList();
    final expenses =
        allItems.where((i) => i.account.accountType == AccountType.expense).toList();
    ...
    for (final item in allItems) {
      accountByCode[item.account.code] = item.totalDebit - item.totalCredit;
    }
    final Decimal costOfGoodsSold = accountByCode['5010'] ?? Decimal.zero;
    final Decimal grossProfit = totalRevenue - costOfGoodsSold;
    final Decimal operatingExpenses = totalExpense - costOfGoodsSold;
    final Decimal netIncome = grossProfit - operatingExpenses;
  ...
  Future<BalanceSheetData> getBalanceSheet({DateTime? date}) async {
    ...
    for (final node in _flattenTree(tree)) {
      ...
      if (node.account.accountType == AccountType.asset) {
        assets.add(BalanceSheetItem(node.account, balance));
      } else if (node.account.accountType == AccountType.liability) {
        liabilities.add(BalanceSheetItem(node.account, balance));
      } else if (node.account.accountType == AccountType.equity) {
      ...
      for (var line in lines) {
        if (!cashAccountIds.contains(line.account.id)) {
          if (line.account.accountType == AccountType.revenue ||
            ...
            categorized = true;
            break;
          } else if (line.account.accountType == AccountType.asset &&
              line.account.code != AccountCodes.cash &&
              line.account.code != AccountCodes.bank &&
              line.account.code != AccountCodes.accountsReceivable) {
            investingActivities += cashMovement;
            categorized = true;
            break;
          } else if (line.account.accountType == AccountType.liability ||
              line.account.accountType == AccountType.equity) {
