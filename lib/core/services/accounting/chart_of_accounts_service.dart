...
class ChartOfAccountsService {
  ...
  Future<void> seedDefaultAccounts({String? branchId}) async {
    ...
    final accounts = {
      ...
      AccountCodes.deferredInputVAT: GLAccountsCompanion.insert(
        code: AccountCodes.deferredInputVAT,
        name: 'ضريبة المدخلات المؤجلة (الأساس النقدي)',
        accountType: AccountType.asset,
        branchId: Value(effectiveBranchId),
      ),
      ...
      AccountCodes.deferredOutputVAT: GLAccountsCompanion.insert(
        code: AccountCodes.deferredOutputVAT,
        name: 'ضريبة المخرجات المؤجلة (الأساس النقدي)',
        accountType: AccountType.liability,
        branchId: Value(effectiveBranchId),
      ),
  ...
  Future<String> createCustomerAccount(String customerName) async {
    ...
    if (parent == null) {
      throw const NotFoundException(
        message: 'حساب الذمم المدينة الرئيسي (${AccountCodes.accountsReceivable}) غير موجود.',
        details: 'يجب إنشاءه أولاً من شجرة الحسابات.',
      );
    }
    ...
    final branch = await (db.select(db.branches)
      ..where((b) => b.id.equals(defaultBranchId)))
        .getSingleOrNull();
    if (branch == null) {
      throw const BusinessException(message: 'الفرع الافتراضي غير موجود. تعذر إنشاء حساب العميل.');
    }
  ...
  Future<String> createSupplierAccount(String supplierName) async {
    ...
    if (parent == null) {
      throw const NotFoundException(
        message: 'حساب الذمم الدائنة الرئيسي غير موجود.',
        details: 'تعذر إنشاء حساب المورد.',
      );
    }
    ...
    final branch = await (db.select(db.branches)
      ..where((b) => b.id.equals(defaultBranchId)))
        .getSingleOrNull();
    if (branch == null) {
      throw const BusinessException(message: 'الفرع الافتراضي غير موجود. تعذر إنشاء حساب المورد.');
    }
    await dao.createAccount(
      GLAccountsCompanion.insert(
        id: Value(id),
