...
class AutoBreakService {
  ...
  Future<void> addUnitHierarchy({
    ...
    if (existing != null) {
      throw BusinessException(message: 'الوحدة "$unitName" موجودة بالفعل لهذا المنتج');
    }

    if (factor <= Decimal.zero) {
      throw const BusinessException(message: 'معامل الوحدة يجب أن يكون أكبر من صفر');
    }
  ...
  Future<void> removeUnitHierarchy({
    ...
    if (product != null && product.unit == unitName) {
      throw const BusinessException(message: 'لا يمكن حذف الوحدة الأساسية');
    }
