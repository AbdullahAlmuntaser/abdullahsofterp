...
class PayrollService {
  ...
  Future<void> paySalaries(String payrollRunId) async {
    ...
    if (payrollRun.status != 'posted') {
      throw const BusinessException(message: 'يجب ترحيل قيد الرواتب أولاً');
    }
  ...
  Future<String> _getSalaryExpenseAccount() async {
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) {
      throw const BusinessException(message: 'لم يتم العثور على حساب مصروفات الرواتب');
    }
  ...
  Future<String> _getDeductionsLiabilityAccount() async {
    ...
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب الخصوم');
    return accounts.first.id;
  }
  ...
  Future<String> _getSalariesPayableAccount() async {
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب المستحقات');
    return accounts.first.id;
  }
  ...
  Future<String> _getBankAccount() async {
    final accounts = await (db.select(db.gLAccounts)
        ...
        .get();
    if (accounts.isEmpty) throw const BusinessException(message: 'لم يتم العثور على حساب البنك');
    return accounts.first.id;
  }
