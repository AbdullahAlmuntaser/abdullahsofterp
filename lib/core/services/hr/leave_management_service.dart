...
class LeaveManagementService {
  ...
  Future<LeaveRequest> createLeaveRequest({
    ...
    if (balance != null && balance.remaining < totalDays) {
      throw BusinessException(message: 'رصيد الإجازات غير كافي. المتبقي: ${balance.remaining} يوم');
    }
    ...
    if (overlapping.isNotEmpty) {
      throw const BusinessException(message: 'يوجد طلب إجازة متداخل مع هذه الفترة');
    }
  ...
  Future<void> approveLeaveRequest({
    ...
    if (request == null) throw const BusinessException(message: 'طلب الإجازة غير موجود');
    if (request.status != 'PENDING') throw const BusinessException(message: 'طلب الإجازة ليس في حالة انتظار');
