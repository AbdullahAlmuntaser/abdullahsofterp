...
class DeliveryNotesService {
  ...
  Future<DeliveryNote> createDeliveryNote({
    ...
    if (salesOrder == null) {
      throw const BusinessException(message: 'أمر البيع غير موجود');
    }

    if (salesOrder.status == SalesOrderStatus.cancelled) {
      throw const BusinessException(message: 'لا يمكن إنشاء توصيل لأمر بيع ملغي');
    }
    ...
    if (orderItems.isEmpty) {
      throw const BusinessException(message: 'أمر البيع لا يحتوي على أصناف');
    }
  ...
  Future<void> completeDeliveryNote(String deliveryNoteId) async {
    ...
    if (deliveryNote == null) {
      throw const BusinessException(message: 'سند التوصيل غير موجود');
    }

    if (deliveryNote.status == 'COMPLETED') {
      throw const BusinessException(message: 'سند التوصيل مكتمل بالفعل');
    }
  ...
  Future<void> cancelDeliveryNote(String deliveryNoteId) async {
    ...
    if (deliveryNote.status == 'COMPLETED') {
      throw const BusinessException(message: 'لا يمكن إلغاء سند توصيل مكتمل. يجب إلغاء التوصيل أولاً.');
    }
