...
class ProformaService {
  ...
  Future<void> convertToSale(String proformaId) async {
    final proforma = await getProforma(proformaId);
    final items = await getProformaItems(proformaId);
    final saleId = const Uuid().v4();

    await db.into(db.sales).insert(SalesCompanion.insert(
          id: Value(saleId),
          customerId: Value<String?>(proforma.customerId),
          total: proforma.total,
          discount: Value(proforma.discount),
          tax: Value(proforma.tax),
          status: const Value(DocumentStatus.posted),
          paymentMethod: PaymentMethod.cash,
          saleType: const Value('invoice'),
    ...
    for (final item in items) {
      await db.into(db.saleItems).insert(SaleItemsCompanion.insert(
            ...
            price: item.price,
            unitId: Value<String?>(item.unitId),
            unitName: Value(item.unitName),
            unitFactor: Value(item.unitFactor),
          ));
    }

    await updateStatus(proformaId, DocumentStatus.received);
  }
}
