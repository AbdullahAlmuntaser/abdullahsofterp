...
class StockOperationService {
  ...
  Future<void> _postInventoryAdjustment(
    ...
    if (inventoryAccount == null || adjustmentAccount == null) {
      throw const BusinessException(message: 'Missing GL accounts for inventory adjustment.');
    }
    ...
    await db.transaction(() async {
      ...
      if (!allowNegative && product.stock < quantity) {
        throw BusinessException(
            message: 'الرصيد الحالي (${product.stock}) غير كافٍ لخصم الكمية ($quantity). العملية مرفوضة.');
      }
