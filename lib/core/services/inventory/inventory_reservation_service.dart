...
class InventoryReservationService {
  ...
  Future<InventoryReservation> createReservation({
    ...
    if (availableStock < reservedQuantity) {
      throw BusinessException(message: 'المخزون المتاح ($availableStock) أقل من الكمية المطلوب حجزها ($reservedQuantity)');
    }
  ...
  Future<void> fulfillReservation({
    ...
    if (reservation == null) throw const BusinessException(message: 'الحجز غير موجود');

    final remaining = reservation.reservedQuantity - reservation.fulfilledQuantity;
    if (fulfilledQuantity > remaining) {
      throw BusinessException(message: 'الكمية المطلوب تخطي ($fulfilledQuantity) تتجاوز المتبقي ($remaining)');
    }
